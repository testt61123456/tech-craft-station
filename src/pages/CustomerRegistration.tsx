...
const CustomerRegistration = () => {
  ...
  // Realtime dinleme - yeni müşteri ve cihaz güncellemeleri
  useEffect(() => {
    ...
    const customersChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          showNotification('Yeni Müşteri Kaydı', `${newCustomer.customer_name} kaydedildi`);
          fetchCustomers();
        }
      )
      .on(
        ...
        (payload) => {
          ...
          showNotification('Yeni Cihaz Kaydı', `Yeni cihaz eklendi`);
          // İlgili müşterinin cihazlarını yenile
          if (device.customer_id && customerDevices[device.customer_id]) {
            fetchCustomerDevices(device.customer_id);
          }
        }
      )
      .on(
        ...
        (payload) => {
          ...
          if (oldDevice.status !== newDevice.status) {
            const statusLabels: Record<string, string> = {
              ...
              warranty_sent: 'Garantiye Gönderildi',
              completed: 'Tamamlandı',
              delivered: 'Teslim Edildi'
            };
            showNotification(
              'Durum Güncellendi',
              `Cihaz durumu: ${statusLabels[newDevice.status] || newDevice.status}`
            );
          }
          
          // Parça bekleme süresi girildiğinde
          if (!oldDevice.waiting_days && newDevice.waiting_days) {
            scheduleWaitingDaysNotification(newDevice.id, newDevice.waiting_days);
          }
          
          // İlgili müşterinin cihazlarını yenile
          if (newDevice.customer_id && customerDevices[newDevice.customer_id]) {
            fetchCustomerDevices(newDevice.customer_id);
          }
        }
      )
      .subscribe();
  ...
  }, [user, userRole, customerDevices]);

  // 7 günden fazla bekleyen cihazları kontrol et
  useEffect(() => {
    if (!user || (userRole !== 'admin' && userRole !== 'superadmin')) return;

    const checkLongWaitingDevices = async () => {
      try {
        ...
        const { data: devices, error } = await supabase
          ...
          .select('id, device_type, received_date, customer_id')
          .eq('status', 'pending')
          .lt('received_date', sevenDaysAgo.toISOString());

        if (!error && devices && devices.length > 0) {
          showNotification(
            '⚠️ Uzun Süre Bekleyen Cihazlar',
            `${devices.length} cihaz 7 günden fazla beklemede`
          );
        }
      } catch (error) {
        console.error('Bekleyen cihazlar kontrol hatası:', error);
      }
    };

    // İlk kontrolü yap
    checkLongWaitingDevices();

    // Her 2 saatte bir kontrol et
    const interval = setInterval(checkLongWaitingDevices, 2 * 60 * 60 * 1000);
  ...
  const showNotification = (title: string, body: string) => {
    ...
    // Aynı zamanda toast bildirimi de göster
    toast.info(body, { description: title });
  };

  // Parça bekleme süresi için geri sayım
  const scheduleWaitingDaysNotification = (deviceId: string, waitingDays: number) => {
    const notificationKey = `waiting_notification_${deviceId}`;
    const endTime = Date.now() + (waitingDays * 24 * 60 * 60 * 1000);
    
    // localStorage'a kaydet
    localStorage.setItem(notificationKey, endTime.toString());
    
    // Süre dolduğunda bildirim göster
    const checkWaitingTime = () => {
      const storedEndTime = localStorage.getItem(notificationKey);
      if (!storedEndTime) return;
      
      const remaining = parseInt(storedEndTime) - Date.now();
      
      if (remaining <= 0) {
        showNotification(
          '⏰ Parça Bekleme Süresi Doldu',
          `Cihaz için belirlenen bekleme süresi sona erdi`
        );
        localStorage.removeItem(notificationKey);
      } else {
        // Kalan süreyi kontrol etmeye devam et
        setTimeout(checkWaitingTime, Math.min(remaining, 60000)); // En fazla 1 dakika bekle
      }
    };
  ...
  // Sayfa yüklendiğinde bekleyen geri sayımları kontrol et
  useEffect(() => {
    const checkAllWaitingNotifications = () => {
      Object.keys(localStorage).forEach(key => {
        if (key.startsWith('waiting_notification_')) {
